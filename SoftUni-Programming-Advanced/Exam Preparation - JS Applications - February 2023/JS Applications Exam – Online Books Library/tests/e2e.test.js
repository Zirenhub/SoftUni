...
describe('E2E tests', function () {
  ...
  describe('Authentication [ 20 Points ]', () => {
    ...
    it('logout makes correct API call [ 5 Points ]', async () => {
      const data = mockData.users[0];
      const { post: loginPost } = await createHandler(
        endpoints.login,
        { post: data }
      );
      const { get: logoutGet } = await createHandler(
        endpoints.logout,
  ...
  describe('Navigation bar [ 5 Points ]', () => {
    it('guest user should see correct navigation [ 2.5 Points ]', async () => {
      ...
      expect(await page.isVisible('nav >> text=Dashboard')).to.be
        .true;
      expect(await page.isVisible('nav >> text=Login')).to.be.true;
      expect(await page.isVisible('nav >> text=Register')).to.be.true;

      expect(await page.isVisible('nav >> text=My Books')).to.be
        .false;
      expect(await page.isVisible('nav >> text=Add Book')).to.be
        .false;
    ...
    it('logged user should see correct navigation [ 2.5 Points ]', async () => {
      ...
      expect(await page.isVisible('nav >> text=Dashboard')).to.be
        .true;
      expect(
        await page.isVisible(`nav >> text=Welcome, ${data.email}`)
      ).to.be.true;
      ...
      expect(await page.isVisible('nav >> text=Register')).to.be
        .false;
    });
  });
  ...
  describe('Catalog [ 20 Points ]', () => {
    ...
    it('show details [ 5 Points ]', async () => {
      ...
      expect(await page.isVisible(`h3:has-text("${data.title}")`)).to
        .be.true;
      expect(await page.isVisible(`.type:has-text("${data.type}")`))
        .to.be.true;
      expect(
  ...
  describe('BONUS : Like functionality  [ 15 Points ]', async () => {
    it('Like button is not visible for guest users [ 2.5 Points ]', async () => {
      ...
      expect(await page.isVisible('.actions .button >> text="Like"'))
        .to.be.false;
    ...
    it('Like button should be hidden(not visible) after a click on it [ 2.5 Points ]', async () => {
      ...
      await page.waitForTimeout(interval);
      expect(await page.isVisible('.actions >> text="Like"')).to.be
        .true;
...
async function setupContext(context) {
  ...
  await createHandler(
    endpoints.own('1001', '0001'),
    { get: 1 },
    context
  );
  await createHandler(
    endpoints.own('1002', '0001'),
    { get: 0 },
    context
  );
  await createHandler(
  ...
  await createHandler(
    endpoints.own('1004', '0002'),
    { get: 0 },
    context
  );
  await createHandler(
    endpoints.own('1005', '0002'),
    { get: 0 },
    context
  );
  // Catalog and Details
...
async function createHandler(match, handlers = {}, context) {
  ...
  ['get', 'post', 'put', 'patch', 'delete'].forEach((method) =>
    Object.defineProperty(result, method, {
      get: () => undefined,
      set: (value) =>
        createMethodHandler(method, ...parseOptions(value)),
      configurable: true,
    })
  );
